...
# if [ "$#" -eq 0 ]; then
#   echo "Collecting all parallel tests except these..."
#   for t in $(cat $TESTS_DIR/sequential_tests.txt); do
# 	filepath=$(find "$TESTS_DIR/$t" -type f)
#   	PT="$PT"$'\n'"$filepath"
#   done
#   final_tests=$(find "$TESTS_DIR/api" "$TESTS_DIR/ui" -name "test_*.py" -type f)!$PT
# else
...
# echo "here's PT out"
# echo $PT
# #final_tests=$(find "$TESTS_DIR/api" "$TESTS_DIR/ui" -name "test_*.py" -not -path "" -type f)
# echo $final_tests

for t in $(cat $TESTS_DIR/parallel_tests.txt); do
	filepath=$(find "$TESTS_DIR/$t" -type f)
  	PT="$PT"$'\n'"$filepath"
done
final_par_tests=$(find $PT -type f)
######################################

set +e
echo "Running PARALLEL TESTS FROM FILE"
echo "$final_par_tests" | parallel --halt-on-error 0 "$PY_TEST '{}' --junitxml='$REPORTS_DIR/{/.}.xml' --url='$STORE_URL' --username='$USERNAME' --email='$EMAIL' --password='$PASSWORD' --browser='$BROWSER' --auth_token='$AUTH_TOKEN' -v -x"
